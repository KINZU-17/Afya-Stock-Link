...
function Home() {
  const { data: facility, loading: fLoading } = useFetch(
    "http://localhost:3001/facility_info",
  );
    ...
    return (
      <div className="flex items-center justify-center py-24">
        <div className="text-center">
          <div className="w-14 h-14 bg-hospital-teal rounded-xl flex items-center justify-center mx-auto text-white text-2xl font-black animate-pulse">
            +
          </div>
          <p className="mt-4 text-hospital-teal font-semibold animate-pulse">
  ...
  const lowStock =
    products?.filter((p) => p.quantity > 0 && p.quantity <= 20).length || 0;
  const totalValue =
    ...
    <div className="space-y-8">
      ...
      <div className="bg-linear-to-r from-hospital-teal to-teal-600 text-white rounded-2xl p-6 shadow-md">
        ...
        <p className="text-teal-100 text-sm mt-1">
          {user?.role} — {facility?.name}
        </p>
        <div className="flex gap-4 mt-4 flex-wrap justify-start items-center">
          <span className="bg-white/20 rounded-lg px-3 py-1 text-xs h-10 font-bold">
            MFL Code: {facility?.facility_code}
          </span>
          <span className="bg-white/20 rounded-lg px-3 py-1 text-xs font-bold">
            <Phone size={16} />{facility?.emergency_contact}
          </span>
          <span className="bg-white/20 rounded-lg px-3 py-1 text-xs font-bold">
            <Building2 size={16} />{facility?.beds} Beds · <Users size={16} />{facility?.staff} Staff
          </span>
        </div>
      </div>
      ...
      {criticalItems.length > 0 && (
        <div className="bg-white rounded-xl p-6 border border-red-100 shadow-sm">
            ...
            {criticalItems.map((item) => {
              ...
              return (
                ...
                >
                  ...
                  <span
                    className={`text-xs font-bold px-2 py-1 rounded-full border ${status.color}`}
                  >
                    {item.quantity} units
                  </span>
                </div>
          ...
          <Link
            to="/products"
            className="mt-4 inline-block text-sm font-bold text-hospital-teal hover:underline"
          >
            View Full Inventory →
          </Link>
        </div>
        ...
        {[
          {
            ...
            sub: "Browse all medical stock",
          },
          {
            to: "/add-product",
            icon: <Plus size={24} />,
            title: "Log Inbound Stock",
            sub: "Register new supply batch",
          },
          {
            to: "/symptom-checker",
            icon: <Stethoscope size={24} />,
            title: "Symptom Checker",
            sub: "AI-powered diagnosis tool",
          },
          {
            to: "/nearby",
            icon: <MapPin size={24} />,
            title: "Nearby Facilities",
            sub: "Find health centers near you",
          },
        ].map((action) => (
          <Link
            key={action.to}
          ...
          >
            <span className="text-3xl flex items-center justify-center">{action.icon}</span>
            <p className="font-bold text-slate-800 mt-2 group-hover:text-hospital-teal transition-colors text-sm">
              {action.title}
            </p>
